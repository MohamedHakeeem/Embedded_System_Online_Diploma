...
void MCAL_UART_SendData	(USART_TypeDef *USARTx, uint16_t *pTxBuffer,enum Polling_mechism PollingEn )
{
	...
	if (Global_UART_Config[0]->Payload_Length == UART_Payload_Length_9B || Global_UART_Config[1]->Payload_Length == UART_Payload_Length_9B
			||Global_UART_Config[2]->Payload_Length == UART_Payload_Length_9B  )
	{
